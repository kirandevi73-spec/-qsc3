...
import {
  Cpu, Lock, Database, Network, Link as LinkIcon,
  Shield, Zap, Activity, CheckCircle, AlertTriangle,
  RefreshCw, ChevronDown, ChevronUp, Server, GitBranch,
  Eye, Layers, Box
} from 'lucide-react';
...
const STAGES = [
  ...
  },
  {
    ...
    badge: 'Compute Root',
    color: '#bc13fe',
    glow: 'rgba(188,19,254,0.25)',
    border: 'rgba(188,19,254,0.45)',
    icon: GitBranch,
...
const TRACE_STEPS = [
  {
    label: '// 1. Raw Telemetry (JSON)',
    color: '#94a3b8',
    value: '{"temp":24.5,"humid":60,"loc":"KHI-WH-3","ts":1748131200}',
    size: '~128 B',
  },
  {
    label: '// 2. Dilithium-3 Signature (hex-truncated)',
    color: '#00f3ff',
    value: '0x7a3f9c2d8e1b4f6a0d5c3e9f2a7b1d4e8c0f3a6...e2d1',
    size: '3.3 KB',
  },
  {
    label: '// 3. IPFS CID (v1 base32)',
    color: '#60a5fa',
    value: 'bafybeigdyrzt5sfp7udm7hu76uh7y26nf3efuylqabf4midvogmr2gbddi',
    size: '46 B',
  },
  {
    label: '// 4. Merkle Root (SHA-256 batch)',
    color: '#bc13fe',
    value: '0x9f2a3e7c1b4d8f0a2c5e9b3d7f1a4c8e2b5d9f3a6c0e4b7d1f5a8c2e6b0d4f8',
    size: '32 B',
  },
  {
    label: '// 5. On-Chain Anchor',
    color: '#4ade80',
    value: 'Block #21,884,502 · TxHash: 0xfe3a...1c9b · Gas: 21,000',
    size: 'FINAL',
  },
];
...
const SECURITY = [
  {
    ...
    color: '#00f3ff',
    icon: Shield,
    body: 'Dilithium-3 (ML-DSA) signatures are CRQC-resistant. Even adversaries with fault-tolerant quantum computers running Shor\'s algorithm cannot forge or repudiate a signed payload.',
  },
  {
    title: 'Data Availability',
    color: '#60a5fa',
    icon: Database,
    body: 'Pinata-pinned IPFS keeps bulky signature blobs off-chain across a distributed content network, guaranteeing retrieval without bloating the ledger.',
  },
  {
    title: 'Batch Efficiency',
    color: '#bc13fe',
    icon: Layers,
    body: 'Merkle batching condenses 4 096 individual transactions into one 32-byte root, achieving >99.99 % gas reduction while preserving full individual auditability via Merkle proof.',
  },
  {
    title: 'Immutability & Auditability',
    color: '#4ade80',
    icon: CheckCircle,
    body: 'The Merkle root anchored on-chain acts as a cryptographic timestamp. Any post-anchor mutation of leaf data is instantly detectable via root mismatch.',
  },
];

// ─── ANIMATED PACKET DOT ─────────────────────────────────────────────────────
function PacketDot({ color, delay }) {
  return (
    <motion.div
    ...
    <div className="space-y-8">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-gray-400 text-sm">
              End-to-end quantum-safe data pipeline · Edge → PQC → IPFS → Merkle → Blockchain
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            {loading ? (
              ...
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-green-500/30 bg-green-500/10 text-green-400 text-xs font-mono">
                ...
                Backend Live
                {blockStats?.totalAnchors != null && (
                  <span className="text-gray-500 ml-1">· {blockStats.totalAnchors} anchors</span>
                )}
              </div>
            )}
          </div>
      ...
      >
        ...
        <div
          ...
          style={{
            backgroundImage:
              'linear-gradient(rgba(0,243,255,0.4) 1px, transparent 1px), linear-gradient(90deg, rgba(0,243,255,0.4) 1px, transparent 1px)',
            backgroundSize: '40px 40px',
          }}
        />

        {/* Animated horizontal rail */}
        ...
        <div className="min-w-[840px] flex justify-between items-start relative z-10 overflow-x-auto pb-2">
          {STAGES.map((stage, i) => (
            <React.Fragment key={stage.id}>
              ...
              />
              {i < STAGES.length - 1 && (
                <Connector color={STAGES[i + 1].color} />
              )}
            </React.Fragment>
          ))}
        </div>
      ...
      <AnimatePresence mode="wait">
        {activeData && (
          <motion.div
            ...
            style={{
              borderColor: activeData.border,
              background: `linear-gradient(135deg, ${activeData.glow}, rgba(0,0,0,0.5))`,
            }}
          >
            <div className="flex items-start gap-4 flex-wrap">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-[10px] font-mono tracking-widest" style={{ color: activeData.color }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          ...
          >
            ...
            {traceOpen ? <ChevronUp size={14} className="text-gray-500" /> : <ChevronDown size={14} className="text-gray-500" />}
          </button>
                  ...
                  {TRACE_STEPS.map((t, i) => (
                    ...
                    >
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div className="break-all leading-relaxed" style={{ color: t.color }}>
                          {t.value}
                        </div>
                        <span className="text-gray-700 shrink-0 text-[9px] mt-0.5">{t.size}</span>
                      </div>
                      {i < TRACE_STEPS.length - 1 && (
                        <div className="text-gray-800 pl-2">↓</div>
                      )}
                    </motion.div>
                  ))}
                </div>

                {/* Live backend stats footer */}
                {blockStats && (
                  <div className="px-5 pb-4 pt-0 border-t border-white/5 mt-1">
                    <div className="flex items-center justify-between text-[10px] font-mono text-gray-500">
                      <span className="text-green-400 flex items-center gap-1">
                        <CheckCircle size={10} /> {blockStats.totalAnchors ?? '—'} total anchors
                      </span>
      ...
      >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {[
            ...
            <div
              key={t.label}
              className="rounded-xl border border-white/8 bg-white/[0.03] px-3 py-3 text-center hover:bg-white/[0.07] transition-colors"
            >
